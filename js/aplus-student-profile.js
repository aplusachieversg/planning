...
(function(){
  "use strict";
  // Page-load guard: never autosave the initial empty UI before the database restore completes.
  window.APLUS_ACADEMIC_PROFILE_RESTORING=true;
  window.APLUS_ACADEMIC_PROFILE_HYDRATED=false;
  ...
  function collect(){
    ...
    const targetEntryYear=read("spEntryYear")||read("entryYear")||"2027";
  ...
  function diagnosticState(base){
    const p=base.profile||{},subjects=arr(p.subjects),analysis=p.academicAnalysis||{},completeness=p.profileCompleteness||{};
    const graded=subjects.filter(x=>x.grade&&x.grade!=="not_available");
    const strong=arr(analysis.classifications).filter(x=>x.classification==="strong");
    const state={
      education:p.currentLevel?"recorded":"unknown",
      qualification:p.qualification?"recorded":"unknown",
      subjects:subjects.length?"recorded":"unknown",
      grades:graded.length?"recorded":"unknown",
      strengths:strong.length?"recorded":"unknown",
      pattern:analysis.pattern&&analysis.pattern.label?"recorded":"unknown",
      development:(p.weakTopics&&p.weakTopics.length)?"recorded":"not_identified"
    };
    const next=[];
    if(state.subjects==="unknown")next.push("Record current or planned subjects.");
    if(state.grades==="unknown")next.push("Add the latest available subject grades.");
    if(!completeness.weakTopics)next.push("Identify the main academic areas to develop.");
    return {state,next};
  }

  function save(profile){try{const previous=JSON.parse(localStorage.getItem("APLUS_MASTER_PROFILE")||"null");if(previous){const history=arr(JSON.parse(localStorage.getItem("APLUS_PROFILE_HISTORY")||"[]"));history.unshift({savedAt:new Date().toISOString(),summary:window.APLUS_MASTER_PROFILE.summary(previous)});localStorage.setItem("APLUS_PROFILE_HISTORY",JSON.stringify(history.slice(0,12)));}localStorage.setItem("APLUS_MASTER_PROFILE",JSON.stringify(profile));localStorage.setItem("APLUS_DIAGNOSTIC_01",JSON.stringify({version:"5.0",completedAt:new Date().toISOString(),state:diagnosticState(profile).state}));}catch(e){}}
  ...
  async function build(){
    ...
    const analysis=base.profile.academicAnalysis||{}, graded=base.profile.subjects.filter(x=>x.grade&&x.grade!=="not_available");
    const strengthItems=arr(analysis.classifications).filter(x=>x.classification==="strong").map(x=>x.level+" "+x.subject);
    const pattern=analysis.pattern?.label||"Not enough data yet";
    const readiness=analysis.readiness?.label||"Not assessed";
    const development=base.profile.weakTopics&&base.profile.weakTopics.length?base.profile.weakTopics.join(", "):"Not identified yet";
    result.style.display="block";
    result.innerHTML='<div class="sp2-header"><div><div class="eyebrow" style="color:#3157ff">DIAGNOSTIC 01 · COMPLETED</div><h3>Academic Profile</h3><p>Student Baseline — a clear view of the student\'s current academic position before target-specific analysis.</p></div><span class="sp2-badge">STUDENT BASELINE</span></div>'+
      '<div class="diag-grid">'+
      '<div class="diag-box"><div class="diag-label">Academic Strength</div><div class="diag-status">'+esc(strengthItems.length?strengthItems.join(", "):"Not enough graded evidence yet")+'</div></div>'+
      '<div class="diag-box"><div class="diag-label">Academic Pattern</div><div class="diag-status">'+esc(pattern)+'</div></div>'+
      '<div class="diag-box"><div class="diag-label">Subject Readiness</div><div class="diag-status">'+esc(graded.length?graded.length+" graded subject"+(graded.length===1?"":"s")+" recorded":"Not assessed")+'</div></div>'+
      '<div class="diag-box"><div class="diag-label">Academic Readiness</div><div class="diag-status">'+esc(readiness)+'</div></div>'+
  ...
  function inject(){
    ...
    async function saveAcademicDraft(){
      ...
      try{
        ...
        if(window.APLUS_ALEVEL_SCORE)base.profile.aLevelScore=window.APLUS_ALEVEL_SCORE.calculate(base.profile.subjects);
        if(window.APLUS_ACADEMIC_ANALYSIS)base.profile.academicAnalysis=window.APLUS_ACADEMIC_ANALYSIS.analyze(base.profile.subjects,read("spAcademic")||"unknown");
        base.profile.subjectGrades=base.profile.subjects.map(x=>({level:x.level,subject:x.subject,grade:x.grade||"not_available"}));
        const db=await window.APLUS_DATABASE.saveAcademicProfileDraft(base);
        if(db&&db.ok){
          base.metadata=base.metadata||{};
          base.metadata.databaseSync="synced";
          save(base);
          if(window.APLUS_PROFILE_STORE&&window.APLUS_PROFILE_STORE.saved)window.APLUS_PROFILE_STORE.saved(db.data||null);
        }
      }catch(e){
    ...
    function attachAcademicAutosaveListeners(){
      ...
      ["spLevel","spQualification","spAcademicProfile"].forEach(function(id){
        ...
        el.addEventListener("input",function(){refreshAcademicOutputs();if(window.APLUS_PROFILE_STORE&&window.APLUS_PROFILE_STORE.markDirty)window.APLUS_PROFILE_STORE.markDirty();queueAcademicDraftSave();});
  ...
  function hydrateSavedProfile(row){
    try{
      ...
      // Restore is read-only: never dispatch a change event here, because the change listener autosaves and can write a transient UI state back to the database.
      ...
      refreshAcademicOutputs();
      persistGradeState();
      if(academicDraftTimer)clearTimeout(academicDraftTimer);
      window.APLUS_ACADEMIC_PROFILE_HYDRATED=true;
