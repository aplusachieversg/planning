...
(function(){
  ...
  function getAnalysis(){const a=read(),labels={initiative:"Initiative",commitment:"Commitment",responsibility:"Responsibility",leadership:"Leadership",thinking:"Thinking & Problem Solving",collaboration:"Collaboration & Communication",resilience:"Resilience & Adaptability",reflection:"Reflection & Self-awareness"};const counts={};Object.keys(labels).forEach(k=>counts[k]=0);a.forEach(x=>{const text=[x.whatIDid,x.whatILearned,x.reflection].join(" ").toLowerCase();if(/initi|start|launch|propos|organis|organized|created|founded|built|led/.test(text)||/founder|leader|organiser|project leader/i.test(x.role||""))counts.initiative++;if(x.ongoing||Number(x.endYear)-Number(x.startYear)>=1)counts.commitment++;if(/leader|organiser|organizer|captain|founder|mentor|representative/i.test(x.role||""))counts.responsibility++;if(/leader|captain|founder|project leader/i.test(x.role||""))counts.leadership++;if(/research|analys|problem|solution|design|debug|strategy|experiment|investigat|evaluate|evidence/i.test(text))counts.thinking++;if(/team|collabor|communicat|coordinate|delegate|mentor|support|feedback|present/i.test(text)||/team|organiser|leader|mentor/i.test(x.role||""))counts.collaboration++;if(/challenge|difficult|obstacle|failure|setback|persist|overcome|adapt|revise|iterate/i.test(text))counts.resilience++;if(x.reflection)counts.reflection++;});function level(n){return n>=3?"Established":n>=1?"Developing":"Not yet evidenced";}const personalQualities=Object.keys(labels).map(k=>({key:k,label:labels[k],development:level(counts[k]),evidenceCount:counts[k]}));const patterns=[];if(a.length>=2)patterns.push("Multiple experience records are present.");if(a.some(x=>x.ongoing))patterns.push("Ongoing commitment is recorded.");if(a.some(x=>/leader|captain|founder|project leader/i.test(x.role||"")))patterns.push("Responsibility or leadership roles are recorded.");if(a.some(x=>x.reflection))patterns.push("Reflection is included in the experience record.");return {count:a.length,personalQualities,patterns,evidenceQuality:a.length?(a.some(x=>x.evidenceType&&x.evidenceType!=="No formal evidence")?"Partially evidenced":"Self-reported"):"No experience recorded"};}
...
(function(){
  ...
  function bootExperience(){
    ...
    if(!mount||!window.APLUSExperienceProfile||typeof window.APLUSExperienceProfile.render!=="function")return false;
    try{
      var renamed=false;
      if(mount.id==="experienceModule"){
        mount.id="experienceProfileMount";renamed=true;
      }
      window.APLUSExperienceProfile.render();
      if(renamed)mount.id="experienceModule";
      return true;
    }catch(e){console.error("APLUS Experience render failed",e);return false;}
  }
  function start(){var tries=0,timer=setInterval(function(){tries++;if(bootExperience()||tries>80)clearInterval(timer);},150);bootExperience();}
