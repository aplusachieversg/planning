...
(function(){
  ...
  async function submit(master, extra, gapAnalysis, roadmap){
    ...
    const payload={
      student_id:clean(master.studentId), student_name:clean(extra.studentName), parent_name:clean(extra.parentName),
      email:clean(extra.email), phone:clean(extra.phone), current_level:clean(master.profile.currentLevel),
      qualification:clean(master.profile.qualification), target_field:clean(master.target.field),
      target_university:clean(master.target.university), target_course:clean(master.target.course),
      entry_year:Number(master.target.entryYear)||null, academic_profile:clean(master.profile.academicProfile),
      subjects:master.profile.subjects||[], activities:master.evidence.activities||[], master_profile:master,
      gap_analysis:gapAnalysis||{}, roadmap:roadmap||[], application_status:"New",
      scholarship_interest:!!master.target.scholarship, consent_given:true
    };
